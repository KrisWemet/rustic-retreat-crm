...
function SidebarContent({ onSignOut, isSigningOut }: { onSignOut: () => void; isSigningOut: boolean }) {
  return <>
    <div className="flex h-16 shrink-0 items-center px-4"><h1 className="text-xl font-bold tracking-tight text-white">Planning Pod</h1></div>
    <div className="px-4 pb-6"><div className="flex items-center gap-3 rounded-lg bg-white/10 p-3 shadow-inner">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-sm font-bold text-[var(--accent)] shadow-sm">S</div>
      <div className="min-w-0 flex-1 overflow-hidden"><p className="truncate text-sm font-medium text-white">Shannon</p><button onClick={onSignOut} className="truncate text-xs text-white/70 hover:text-white hover:underline flex items-center gap-1">{isSigningOut ? 'Signing out...' : 'Log out'}</button></div>
    </div></div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="fixed inset-y-0 left-0 z-50 w-[var(--sidebar-w)] bg-[var(--accent)] shadow-xl transition-transform">
            ...
            <SidebarContent onSignOut={handleSignOut} isSigningOut={isSigningOut} />
          </div>
        </div>
      )}
